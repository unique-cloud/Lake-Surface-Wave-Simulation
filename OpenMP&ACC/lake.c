...
#ifdef _OPENMP
  print_heatmap("lake_f_omp.dat", u_cpu, npoints, h);
#elif defined _OPENACC
  print_heatmap("lake_f_acc.dat", u_cpu, npoints, h);
#else
  print_heatmap("lake_f_serial.dat", u_cpu, npoints, h);
#endif
...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
  ...
  //     int block_size = n * n / nthreads;
  //     int offset = block_id * block_size;
  // 
  //     memcpy(uo + offset, u0 + offset, sizeof(double) * block_size);
  //     memcpy(uc + offset, u1 + offset, sizeof(double) * block_size);
  // }
  ...
  #pragma acc data copyin(un[:n*n], uc[:n*n], uo[:n*n], pebbles[:n*n])
  while(1)
  {
    /* run a central finite differencing scheme to solve
     * the wave equation in 2D */
    #pragma omp parallel for num_threads(nthreads) private(i, j, idx)
...
#ifdef _OPENACC
    #pragma acc update host(un[:n*n])    
    memcpy(uo, uc, sizeof(double) * n * n);
    memcpy(uc, un, sizeof(double) * n * n);
    #pragma acc update device(uo[:n*n], uc[:n*n])
#elif _OPENMP
    double* temp = uo;
    uo = uc;
    uc = un;
